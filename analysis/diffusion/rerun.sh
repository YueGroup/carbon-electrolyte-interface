...
WORKDIR=/home/fs01/npn25/projects/automation_run/trial1_2_diffusion
INPUT=rerun_msd.in
...
for dumpfile in ${SIM_DIR}/dump_prod2_wrapped/dump_prod2_wrapped_*.lammpstrj; do
    ...
    wallvars_file="${WORKDIR}/wall_vars.txt"
    ...
    if [[ ! -s "$wallvars_file" ]]; then
        echo "[ERROR] wall_vars.txt is empty. Skipping $expname"
        continue
    fi
